...
# Logging setup
log_file_path = '/home/ubuntu/Poksin_FastApi/fastapi.log'
logging.basicConfig(
    level=logging.INFO,
...
async def detect_violence_in_video(background_tasks: BackgroundTasks, request: EvidenceRequest, db: Session = Depends(get_db)):
    try:
        ...
        logging.error(f"Error starting background task: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_result(evidence_id: int):
    try:
        ...
        if not os.path.exists(result_file_path):
            logging.warning(f"Result file not found: evidence_id={evidence_id}")
        ...
        logging.info(f"Deleted result file: {result_file_path}")
